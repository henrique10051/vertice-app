...
import { TrendingUp, TrendingDown, Wallet, ClipboardList, LineChart, CreditCard } from 'lucide-react'
...
import { InstallmentPurchasesCard } from '@/components/InstallmentPurchasesCard'
      ...
      <Tabs defaultValue="conta">
        <TabsList>
          ...
          <TabsTrigger value="previsao" className="gap-1.5">
            <ClipboardList size={15} /> Previsão
          </TabsTrigger>
          <TabsTrigger value="parcelado" className="gap-1.5">
            <CreditCard size={15} /> Parcelado
          </TabsTrigger>
          <TabsTrigger value="projecao" className="gap-1.5">
            <LineChart size={15} /> Projeção
          </TabsTrigger>
        </TabsList>

        <TabsContent value="conta" className="space-y-8 mt-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="relative overflow-hidden rounded-lg bg-primary text-primary-foreground border-none shadow-elevation">
              ...
              <CardContent className="relative p-6">
                ...
                <div className="data-num text-4xl font-bold tracking-tight">
                  {formatBRL(balance, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-6">
                ...
                <div className="data-num text-3xl font-bold tracking-tight text-chart-4">
                  + {formatBRL(income, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-6">
                ...
                <div className="data-num text-3xl font-bold tracking-tight text-destructive">
                  − {formatBRL(expense, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </div>
              </CardContent>
            </Card>
          </div>
              ...
              <CardContent className="flex flex-col items-center">
                {expensesByCategory.length > 0 ? (
                  ...
                  <div className="h-64 flex flex-col items-center justify-center gap-1 text-center px-6">
                    <p className="text-sm font-medium">Nenhuma despesa ainda</p>
                    <p className="text-xs text-muted-foreground">
                      Registre uma transação para ver a distribuição por categoria.
                    </p>
                  </div>
                )}
        ...
        <TabsContent value="previsao" className="mt-6 space-y-6">
          <BudgetPlanningCard />
          <CategoryManagerCard />
        </TabsContent>

        <TabsContent value="parcelado" className="mt-6 space-y-6">
          <InstallmentPurchasesCard />
          <CategoryManagerCard />
        </TabsContent>
